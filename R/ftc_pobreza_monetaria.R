  ...
  left_join_tipo_cambio() %>%
    dplyr::mutate(
      ...
      ing_mensual_ocup_prin_asalariado = dplyr::case_when(
        ...
        periocupprin == 1 ~ diasocupprin * ingasal * 4.3,
        periocupprin == 2 ~ ingasal * 4.3,
        periocupprin == 3 ~ ingasal * 2,
        periocupprin == 4 ~ ingasal,
        SUELDO_BRUTO_AP == 2 ~ dplyr::if_else(is.na(SALARIO_PRINC_IMP_MONTO), 0, SALARIO_PRINC_IMP_MONTO),
        TRUE ~ 0
      ),
    ...
    dplyr::mutate(
      ...
      ing_mensual_ocup_prin_cuenta_propia = case_when(
        ...
        INGRESO_ACTIVIDAD_IN == 2 ~ dplyr::if_else(
          is.na(GANANCIA_PRINC_IMP_MONTO), 0, GANANCIA_PRINC_IMP_MONTO
        ),
        TRUE ~ 0
      ),
  ...
  tbl %>%
    dplyr::mutate(
      ing_dividendos = dplyr::case_when(
        ...
        DIVIDENDOS_AP_MONTO >= 0 ~ DIVIDENDOS_AP_MONTO / 12,
        TRUE ~ 0
      ),
  ...
  tbl %>%
    dplyr::mutate(
      ing_utilidades_empresariales = dplyr::case_when(
        ...
        UTILIDAD_EMPRESARIAL_AP_MONTO >= 0 ~ UTILIDAD_EMPRESARIAL_AP_MONTO / 12,
        TRUE ~ 0
      ),
  ...
  tbl %>%
    dplyr::mutate(
      ing_beneficios_marginales = dplyr::case_when(
        ...
        BENEFICIOS_MARGINALES_AP_MONTO >= 0 ~ BENEFICIOS_MARGINALES_AP_MONTO / 12,
        TRUE ~ 0
  ...
  tbl %>%
    mutate(
      ing_especie_otros = case_when(
        ...
        OTROS_ESPECIE_AP == 1 ~ OTROS_ESPECIE_AP_MONTO,
        TRUE ~ 0
  ...
  tbl %>%
    mutate(
      perisecctaprop = ifelse(
        is.na(INGRESO_ACTIVIDAD_IS_PERIODO), 0, INGRESO_ACTIVIDAD_IS_PERIODO
      ),
      diassecctaprop = ifelse(
        is.na(INGRESO_ACTIVIDAD_IS_DIAS), 0, INGRESO_ACTIVIDAD_IS_DIAS
      ),
      ing_mensual_ocup_sec_cuenta_propia = case_when(
        ...
        INGRESO_ACTIVIDAD_IS == 2 ~ ifelse(
          is.na(GANANCIA_SECUN_IMP_MONTO), 0, GANANCIA_SECUN_IMP_MONTO
        ),
        TRUE ~ 0
  ...
  tbl %>%
    mutate(
      ing_intereses_dividendos_anual = case_when(
        ...
        INTERESES_NAC_ANO == 1 ~ INTERESES_NAC_ANO_MONTO / 12,
        TRUE ~ 0
  ...
  tbl %>%
    mutate(
      ing_alquileres_renta_anual = case_when(
        ...
        ALQUILER_NAC_ANO == 1 ~ ALQUILER_NAC_ANO_MONTO / 12,
        TRUE ~ 0
...
ftc_ing_transferencias_sociales <- function(tbl, deflactar = TRUE) {
   ...
   tbl<- tbl %>% mutate(
      comeresprimero = case_when(
        ...
        GOBIERNO_NAC == 1 ~ GOB_COMER_PRIMERO_MONTO,
        TRUE ~ 0
      ),
