...
Para el procesamiento de la encuesta puedes utilizar las funiones suministradas de la forma convencional o utilizar operadores pipe como `magrittr::%>%` o `base::|>`.

Como se mencionó antes la ENCFT es una encuesta trimestral por lo que el factor de expansión<!-- ^[Si quieres leer sobre como trabajar con datos de encuesta en R. Lee la siguiente publicación: []()] --> es trimestral.
```{r}
encft %>%
  group_by(TRIMESTRE) %>%
...
```{r}
encft %>%
  ftc_factor_expansion_anual() %>%
  group_by(ANO) %>%
  summarise(
    sum(FACTOR_EXPANSION),
    sum(factor_expansion_anual)
  )
```
...
También puedes asignar etiquetas de datos a las variables utilizando `ftc_set_Dict()`. Consulta la documentación del paquete [labeler](https://adatar-do.github.io/labeler/) para más información.
...
encft %>%
  ftc_factor_expansion_anual() %>%
  group_by(ANO, SEXO) %>%
  summarise(total = sum(factor_expansion_anual), .groups = "drop")

# Con etiquetas aplicadas
encft %>%
  ftc_set_Dict() %>%
  ftc_factor_expansion_anual() %>%
  group_by(ANO, SEXO) %>%
...
Al usar `ftc_set_Dict()`, las variables numéricas como `SEXO` mantienen sus valores pero incluyen metadatos con las etiquetas correspondientes (1 = "Hombre", 2 = "Mujer").
